...
def class_list(request):
    return render(request, 'maneu_class/class_list.html', {'classList': service.class_list(user_id=request.session.get('id'))})
...
def class_delete(request):
    if request.method == 'POST':
        class_insert = service.class_insert(user_id=request.session.get('id'), id=request.POST.get('id'))
        return redirect(reverse('maneu_class:class_list'))
    return render(request, 'maneu_class/class_insert.html')

...
def detail(request):
    ...
    subjective = ManeuRefraction_all(guess_id=guess.id)
    stand_ax = '24.0'
    subjective_refraction = {'OS_VA': 0.2, 'OS_SPH': 0, 'OS_CYL': 0, 'OS_AX': 0, 'OS_AK': 0, 'OS_AL': 16, 'OS_BCVA': 0,
                             'OS_AD': 0, 'OS_CCT': 0, 'OS_LT': 0, 'OS_VT': 0, 'OD_VA': 0.2, 'OD_SPH': 0, 'OD_CYL': 0,
                             'OD_AX': 0, 'OD_AK': 0, 'OD_AL': 16, 'OD_BCVA': 0, 'OD_AD': 0, 'OD_CCT': 0, 'OD_LT': 0,
                             'OD_VT': 0, 'remark': ''}

    return render(request, 'maneu_guest/detail.html',
                  {'guess': guess, 'vision': vision, 'server': server, 'subjective': subjective,
                   'subjectiverefraction': subjective_refraction, 'stand_ax': stand_ax})
...
def insert(request):
    if request.method == 'POST':
        ...
        guess = ManeuGuess_insert(admin_id=request.session.get('id'), time=guess_form['time'], name=guess_form['name'],
                                  phone=guess_form['phone'], sex=guess_form['sex'], age=guess_form['age'],
                                  ot=guess_form['OT'], em=guess_form['EM'], dfh=guess_form['DFH'],
                                  remark=guess_form['remark'])[0]
        if request.POST['subjective'] != subjective_null:
            subjective = ManeuSubjectiveRefraction_insert(admin_id=request.session.get('id'), guess_id=guess.id,
                                                          time=guess.time, content=request.POST['subjective'])
...
def update(request):
    if request.method == 'POST':
        ManeuGuess = ManeuGuess_update(id=request.POST['guess_id'], time=request.POST['time'],
                                       name=request.POST['name'], phone=request.POST['phone'],
                                       sex=request.POST['sex'], age=request.POST['age'], ot=request.POST['OT'],
                                       em=request.POST['EM'], dfh=request.POST['DFH'],
                                       remark=request.POST['remark'])
        request.GET._mutable = True
        request.GET['guess_id'] = request.POST['guess_id']
        request.GET._mutable = False
        return detail(request)
    guess = ManeuGuess_id(admin_id=request.session.get('id'), id=request.GET.get('guess_id'))

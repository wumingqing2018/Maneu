...
def detail(request):
    ...
    order = service.ManeuOrder_id(
        id=order_id, admin_id=request.session.get('id'))
...
def update(request):
    ...
    if request.method == 'POST':
        service.ManeuVision_update(id=request.POST.get(
            'vision_id'), content=request.POST.get('vision_form'))
        service.ManeuStore_update(id=request.POST.get(

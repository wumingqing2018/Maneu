...
def index(request):
    ...
    if admin_id and start and end:
        try:
            data = service.store_index(admin_id=admin_id, start=start, end=end).values('id', 'name', 'call', 'time')
...
def search(request):
    ...
    if admin_id:
        try:
            data = service.store_search(text=text, admin_id=admin_id).values('id', 'name', 'phone', 'time', 'remark')
            content = {'status': True, 'message': '', 'data': list(data)}
        except Exception as e:
            content = {'status': False, 'message': str(e), 'data': {}}
    else:
...
def delete(request):
    ...
    if admin_id and id:
        try:
            data = service.store_delete(admin_id=admin_id, id=id)
...
def insert(request):
    ...
    if admin_id and guest_id:
        ...
        try:
            report = service.store_insert(admin_id=admin_id,
                                           guest_id=guest_id,
...
def insert(request):
    ...
    if admin_id and guest_id:
        try:
            store = service.store_insert(admin_id=admin_id, guest_id=guest_id, time=request.GET.get('time'), content=request.GET.get('store'))
